...
| B8 | Explore filter pills overflow / horizontal-scroll | In the v0.2.25 results header the Month/Distance/Type pills don't fit one row, so the third scrolls off-screen (hidden filter = bad UX). **Fix:** `_pillsRow` (`map_screen.dart`) now shows each filter's *name* at rest (`Month` / `Distance` / `Type`) and its *value* only when active (`Jun 2026` / `10K` / `Races`), and the horizontal `ListView` was replaced with a **`Wrap`** so an overflowing pill drops to a second line. ⚠️ **Fixed in code, NOT yet confirmed on-device** — a tester still saw the pills scrolling after installing v0.2.27. A `Wrap` cannot scroll horizontally, so the running binary likely didn't match the build (stale build, or a debug-vs-release signing mismatch silently refusing the update). v0.2.29 adds an on-screen `build x.y.z` marker to settle which binary is actually installed. **Confirmed fixed on-device 2026-06-22** (pills wrap correctly on the v0.2.31 build). | S | ✅ Resolved — confirmed on-device |
| B9 | Map pins never update with filters | In **map view**, changing Distance (any value — confirmed multiple), and likely Month/Type, leaves stale/extra pins while the **list is correct** (e.g. Distance=Marathon → 3 pins, list shows 1). Cause: markers were maintained in a persistent `Set _markers` and handed to `GoogleMap` as the **same instance** each build — a known `google_maps_flutter` gotcha. **Fixed:** dropped the cached `_markers`/`_circles` fields; markers/circles are now derived from the same filtered `_buildResults()` the list uses and passed to `GoogleMap` as a **fresh `Set`** each build (`_markersFor` / `_circlesFor`), so map & list can't disagree. | S | ✅ Resolved |
| B10 | Reviews count not updating | The race-detail **hero header** ("avg · N reviews") read `race.reviewCount`/`averageRating` from the one-shot, 2-min-cached `getRace` future, so it lagged a just-posted review (and a race's *first* review didn't show until reopening). The "Reviews (N)" section header and profile Reviews stat were already live (streams). **Fixed:** the header rating is now a `_HeaderRating` widget streaming `raceReviews`, computing avg + count live. | S | ✅ Resolved |
| B11 | Calendar day panel caps runs/people at 2 + needs redesign | On **Plan** (`calendar_screen.dart`), the day panel reused the compact list row (≤3 avatars + a terse "Alice +2 going" label). **Fixed:** new `_DayRaceCard` — one card per race (no cap on races, already the case), dropping the redundant date column and listing **every** attendee (You + each pal, avatar + name); a long list collapses behind an "and N more" toggle so a busy day doesn't force endless scrolling. | M | ✅ Resolved |
...
| 16 | Export races to Google Calendar | Let a user add a race they're attending to their own Google Calendar. Wired: **"Add to Google Calendar"** button on race detail → `AppProvider.addRaceToCalendar(race, {on})` → `GoogleCalendarService` (reuses the existing Google sign-in, prompts for the `calendar.events` scope on first use). **v0.2.31: parkruns now supported** — the button shows on parkruns too and asks "Which Saturday?" (`pickParkrunSaturday`) before exporting that date; confirmation snackbar names the date. **Export confirmed working on-device** (v0.2.31). Scope **added to the production OAuth screen without submitting verification** — runs under the unverified-app grace (≤100 testers see a one-time warning). **OAuth verification still deferred to before public launch** (free; sensitive not restricted scope — see PLAY_STORE_LAUNCH.md §9). | S | ✅ Done (v0.2.31) — OAuth verification deferred to launch |
