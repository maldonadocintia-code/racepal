...
| B8 | Explore filter pills overflow / horizontal-scroll | In the v0.2.25 results header the Month/Distance/Type pills don't fit one row, so the third scrolls off-screen (hidden filter = bad UX). **Fix:** `_pillsRow` (`map_screen.dart`) now shows each filter's *name* at rest (`Month` / `Distance` / `Type`) and its *value* only when active (`Jun 2026` / `10K` / `Races`), and the horizontal `ListView` was replaced with a **`Wrap`** so an overflowing pill drops to a second line. | S | ✅ Resolved |
