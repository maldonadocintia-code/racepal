...
- Current version: **v0.2.32-beta** (pubspec `0.2.32+33`) — **faster, complete account deletion** (see Changelog): deletion now runs all personal-data queries together and deletes every matched doc **concurrently** (was one-at-a-time, in series, which dragged on-device), gathering per-doc failures so one denied/missing doc can't abort the rest — but still throwing if any failed, so erasure is complete or fails loudly, never silently partial. **The leftover-sent-pal-requests leak was fixed separately by deploying the `pal_requests` delete rule to production** (PLAY_STORE_LAUNCH.md step 4 — that deploy had been long pending, so deletion threw partway and left sent requests behind). ⚠️ **Not yet re-tested on-device** (needs a fresh v0.2.32 build, full uninstall + reinstall). v0.2.31: **parkruns can now be exported to Google Calendar** (confirmed working on-device): the "Add to Google Calendar" button now shows on parkruns too and asks "Which Saturday?" before adding that one 9am event; the confirmation snackbar names the date. v0.2.30 refreshed the sign-in screen (Wave logo + new copy). v0.2.29 added the **Wave** launcher logo (v0.2.28) + a **temporary on-screen build marker** under the Explore filter pills (a B8 diagnostic — `build x.y.z` from `AppConstants.appVersion`; **remove once B8 is confirmed on-device**) on top of v0.2.27 Explore/Plan bug fixes (BACKLOG B8–B11) — filter pills wrap, map markers track filters, race-detail rating is live, calendar day panel lists all attendees. ✅ **B8 (pill wrap) is now CONFIRMED fixed on a real device** (2026-06-22) — the pills wrap correctly. **Next step: remove the temporary build marker (BACKLOG C2).** Earlier: v0.2.26 Google Calendar export (BACKLOG #16); v0.2.25 Explore "Find your next race" redesign (BACKLOG #14). Volt & Velocity rebrand shipped in v0.2.20 (see "Design system" section).
...
| `lib/services/google_calendar_service.dart` | Google Calendar export (v0.2.26; parkrun support v0.2.31) — `addRace(Race, {DateTime? on})` reuses the existing Google sign-in and prompts for the `calendar.events` scope on first use, then inserts the event into the user's primary calendar. `on` overrides the race's own date (used for the parkrun Saturday picker). Wired via `AppProvider.addRaceToCalendar(race, {on})` → the "Add to Google Calendar" button on race detail. |
...
- **Account deletion** (GDPR erasure) — Me tab → *Delete account*. `AuthService.deleteAccount`/`reauthenticateWithGoogle`, orchestrated by `AppProvider.deleteAccount`. Re-auths via Google, then wipes reviews, attendances, activity, pals (both mirror docs), pal requests, legacy follows, profile photo, user doc, then the auth account. **v0.2.32:** the queries run together (`Future.wait`) and the matched docs are deleted **concurrently** (was sequential/slow); per-doc failures are gathered (one denied/missing doc can't abort the rest) and **thrown at the end**, so erasure is complete or loud — and on failure it stops *before* the profile doc/auth account, keeping the account recoverable for a retry. Still **individual deletes** (not a `WriteBatch`) to respect the batch-delete-of-missing-doc gotcha. The `pal_requests` + `users` self-delete rules are **deployed to production** (2026-06-23).
...
**Pending (user/console actions — see PLAY_STORE_LAUNCH.md):** generate keystore + `key.properties`; `flutter build appbundle`; create Play dev account ($25); upload `.aab`; Data Safety + content rating + listing. (Privacy policy hosting ✅ done — live on GitHub Pages; **Firestore rules ✅ deployed to production 2026-06-23**.)
...
- **v0.2.32** — **Faster, complete account deletion.** `AuthService._deleteUserData` previously deleted docs one-at-a-time across collections in series (`_deleteByQuery` loop), which dragged noticeably on-device. It now runs all nine personal-data queries together (`Future.wait`) and deletes every matched doc **concurrently**. Also hardened: each delete is wrapped so one denied/missing doc can't abort the rest, failures are collected and **thrown at the end** (erasure is complete or fails loudly, never silently partial), and on failure it stops **before** the profile doc/auth account so the account stays recoverable for a retry. Still individual deletes, not a `WriteBatch` (batch-delete-of-missing-doc gotcha). **Separately — and this was the real fix for *sent pal requests surviving deletion* — the `pal_requests` delete rule was deployed to production** (the long-pending PLAY_STORE_LAUNCH.md step 4; without it deletion threw partway and left them behind). `flutter analyze` clean; committed + pushed on `docs/cost-model-ios-web`. ⚠️ **Not yet re-tested on-device** (needs a fresh v0.2.32 build, full uninstall + reinstall).
- **v0.2.31** — **Parkruns can be added to Google Calendar** (confirmed working on-device). The "Add to Google Calendar" button on race detail is no longer gated to dated races — it now shows on parkruns too. Because parkruns recur weekly with no single committed date, tapping it first reuses `pickParkrunSaturday` ("Which Saturday are you running?"), then exports that one 9am event. Threaded an optional `on` date through `GoogleCalendarService.addRace(race, {on})` and `AppProvider.addRaceToCalendar(race, {on})` (no new model boilerplate). The confirmation snackbar now names the date for **all** races (e.g. "Added to your Google Calendar — Sat 4 Jul"). Client-side only; no Firestore/rules changes. `flutter analyze` clean. GitHub release `v0.2.31-beta` (latest), `app-arm64-v8a-release.apk` attached.
- **v0.2.30** — **Refreshed sign-in screen.** `login_screen.dart` now shows the **Wave logo** (`assets/app_logo.png`) in place of the old bolt badge, three emoji feature rows (🏃 Find and plan your next race or parkrun / 🤝 Meet runners like you / ⏱️ Share the experience, not just the time), and the tagline *"Your UK running community"*. `_featureRow` takes an emoji string (34×34 badge) + `Expanded` text. No functional change elsewhere. GitHub release `v0.2.30-beta`.
