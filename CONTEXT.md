...
- Current released version: **v0.2.24-beta** (pubspec `0.2.24+25`) — Explore distance filter (BACKLOG #3): Option A "filters in a sheet" redesign (location pill + Filters button + bottom sheet with Radius/Type/Distance + removable active-filter chips). Volt & Velocity rebrand shipped in v0.2.20 (see "Design system" section).
