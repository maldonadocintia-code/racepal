...
- Current version: **v0.2.27-beta** (pubspec `0.2.27+28`) — Explore/Plan bug fixes (BACKLOG B8–B11): filter pills wrap, map markers track filters, race-detail rating is live, calendar day panel lists all attendees. Previous: v0.2.26 Google Calendar export (BACKLOG #16): "Add to Google Calendar" button on race detail. Earlier: v0.2.25 Explore "Find your next race" redesign (BACKLOG #14): launcher (By location / By month / By distance) → calm results list with location pill + radius slider + quiet Month/Distance/Type filter pills. New Month filter; location optional. Volt & Velocity rebrand shipped in v0.2.20 (see "Design system" section).
